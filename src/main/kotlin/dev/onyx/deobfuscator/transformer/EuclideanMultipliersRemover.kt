...
class EuclideanMultipliersRemover : Transformer {
                    ...
                    when (insn.desc) {
                        INT_TYPE.descriptor -> {
                            when (insn.next.opcode) {
                                I2L -> insnList.insertSafe(insn.next, LdcInsnNode(Inversion.invert(decoder)), InsnNode(LMUL))
                                else -> insnList.insertSafe(insn, LdcInsnNode(Inversion.invert(decoder.toInt())), InsnNode(IMUL))
                            }
                        }
                        LONG_TYPE.descriptor -> insnList.insertSafe(insn, LdcInsnNode(Inversion.invert(decoder)), InsnNode(LMUL))
                        else -> error(insn)
                    }
                }
                PUTFIELD -> {
                        ...
                        INT_TYPE.descriptor -> {
                            when (insn.previous.opcode) {
                                DUP_X1 -> {
                                    ...
                                    insnList.insertSafe(insn, LdcInsnNode(Inversion.invert(decoder.toInt())), InsnNode(IMUL))
                                }
                        ...
                        LONG_TYPE.descriptor -> {
                            when (insn.previous.opcode) {
                                DUP2_X1 -> {
                                    ...
                                    insnList.insertSafe(insn, LdcInsnNode(Inversion.invert(decoder)), InsnNode(LMUL))
                                }
